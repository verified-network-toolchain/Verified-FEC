...
Module Info.
  ...
  Definition source_file := "sumarray_ptr.c"%string.
...
Definition _curr : ident := 55%positive.
Definition _end : ident := 56%positive.
Definition _main : ident := 59%positive.
Definition _n : ident := 54%positive.
Definition _s : ident := 57%positive.
Definition _sumarray : ident := 58%positive.
...
Definition f_sumarray := {|
  ...
  fn_vars := nil;
  fn_temps := ((_curr, (tptr tuint)) :: (_end, (tptr tuint)) ::
               (_s, tuint) :: (_t'1, tuint) :: nil);
  fn_body :=
(Ssequence
  (Sset _curr (Etempvar _a (tptr tuint)))
  (Ssequence
    (Sset _end
      (Ebinop Oadd (Etempvar _a (tptr tuint)) (Etempvar _n tint)
        (tptr tuint)))
    (Ssequence
      (Sset _s (Econst_int (Int.repr 0) tint))
      (Ssequence
        (Swhile
          (Ebinop Olt (Etempvar _curr (tptr tuint))
            (Etempvar _end (tptr tuint)) tint)
          (Ssequence
            (Ssequence
              (Sset _t'1 (Ederef (Etempvar _curr (tptr tuint)) tuint))
              (Sset _s
                (Ebinop Oadd (Etempvar _s tuint) (Etempvar _t'1 tuint) tuint)))
            (Sset _curr
              (Ebinop Oadd (Etempvar _curr (tptr tuint))
...
(_sumarray :: ___builtin_debug :: ___builtin_write32_reversed ::
