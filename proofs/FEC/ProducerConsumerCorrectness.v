Require Import Producer.
Require Import ConsumerMachine.
...
Theorem all_decoded_in_Z: forall (orig : list packet) (encoded received: list fpacket)
  ...
  encoded = (producer_all_steps orig enc_params).2 ->
  (forall p, p \in received -> p \in encoded) ->
  decoded = (consumer_all_steps_gen upd_time not_timeout received).1.2 ->
...
Proof.
  ...
  (*Step 1: since p is in the Consumer, it must have been in [rse_consume_func] at some point*)
  move: Hind. rewrite consumer_all_steps_concat // in_mem_In in_concat => [[dec]] [Hinp].
  ...
  have Hwfenc: wf_packet_stream (producer_all_steps orig enc_params).2 by apply rse_produce_stream_wf.
  ...
  apply in_consume_func_in_block in Hp =>//.
  case : Hp => [[Hp Hpar] | [b1 [b2 [Hinb2 [Hsub [Hrec Hpin]]]]]].
  - (*In the first case, this is easy, since all data packets produced by the Producer are in the original stream*)
    exists p. split => //. rewrite -Hp. apply (rse_produce_stream_from_orig Hencparams) => //.
  ...
  - (*In the second case, we use lots of previous results. Namely, we need to know that b is a subblock of
      ...
      since the subblock in the Consumer is a subblock of a block in (get_blocks received), which is a subblock
    ...
    have [b' [Hsubb' [Hwfb' [Hencb' Hgetb]]]]: exists b', subblock b1 b' /\ block_wf b' /\ block_encoded b' /\
      b' \in (get_blocks (producer_all_steps orig enc_params).2) . {
      ...
      - by apply (rse_produce_stream_blocks_wf Hencparams Hallvalid).
      - apply (rse_produce_stream_recoverable_encoded Hencparams Hallvalid) => //.
    ...
    apply (rse_produce_stream_from_orig Hencparams) => //.
...
Theorem block_i_recovered_Z (orig : list packet) 
  ...
  decoded = (consumer_all_steps threshold received).1.2 ->
...
Proof.
  ...
  (*First, we use [producer_concat_nochange_eq] to reason about
    the Producer with no parameter changes*)
  have Henceq: enc_params = zseq (Zlength orig) (k, h) by
    apply zseq_eq.
  rewrite Henceq in Henc. clear Hlenenc Hallkh Henceq enc_params.
  move: Henc. rewrite producer_all_steps_concat 
    -producer_concat_nochange_eq => Henc.
  (*Next, use [producer_boundaries_i] to get the boundary*)
  have:=(producer_boundaries_i k_bound h_bound 
    Hallvalid Hallenc Hseqnum Hinp Hi)=>/=.
  rewrite -Henc.
  set (b := nth block_inhab (rev (producer_concat_nochange k h orig).1.1) i).
  move=> [Hinb [Hencb [Hwfb [Hbk [Huniq [Hall [Hszenc [f [Hpf [Hparf Hinfb]]]]]]]]]].
  (*Now, we use [consumer_timeout_notimeout_all] to show that the
    Consumer is equivalent to one without timeouts. We first need
    the preconditions:*)
  have Hwf: wf_packet_stream encoded. {
    (*ugh, dont want to reverse*)
    rewrite Henc producer_concat_nochange_eq -producer_all_steps_concat.
    apply rse_produce_stream_wf=>//.
  ...
  move: Hdec. rewrite (consumer_timeout_notimeout_all Hwf' Hreordup) 
    => Hdec.
...
(*One more corollary, combining the two previous ones:
...
(*Doesn't even need anything about Producer*)
Theorem all_data_sent: forall (threshold: int) 
  (received: list fpacket) (p: fpacket),
  p \in received ->
  fd_isParity p = false ->
  p_packet p \in (consumer_all_steps_m threshold received).1.2.
Proof.
...
(*Intermediate spec: if the FEC params are within the correct bounds
  ...
  Consumer only outputs packets that were originally given to the Producer,
...
Theorem all_decoded_in (orig : list packet) 
  ...
  encoded = (producer_all_steps orig enc_params).2 ->
  (forall p, p \in received -> p \in encoded) ->
  decoded = (consumer_all_steps_m threshold received).1.2 ->
...
Proof.
  ...
  rewrite Hdec consumer_all_steps_m_eq //.
  move=> p' Hinp'.
...
Proof.
  ...
  rewrite Hdec consumer_all_steps_m_eq //; try lia.
  move=> p' Hinp'.
