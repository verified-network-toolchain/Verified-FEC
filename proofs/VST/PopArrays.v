...
Proof.
  rewrite /pop_find_lost /= /find_lost /=. f_equal. list_solve.
Qed.

Lemma cat_app {A : Type} (l1 l2: list A) : (l1 ++ l2)%list = l1 ++ l2.
Proof. by []. Qed.
...
Proof.
  rewrite /pop_find_found /= /find_found /=. f_equal. list_solve.
Qed.
...
Proof.
  rewrite /pop_find_parity_rows /= /find_parity_rows /=. f_equal. list_solve.
Qed.
...
Proof.
  ...
  rewrite cat_app upd_Znth0. f_equal. f_equal. f_equal.
  rewrite Zlength_app; list_solve.
Qed.
...
Proof.
  ...
  - rewrite upd_Znth_app2; rewrite !Zlength_map; [| list_solve].
    ...
    Zlength (find_found pack found) - Zlength (find_parity_found pars max_n k))
     = 0%Z by lia.
    symmetry. rewrite zseq_hd. rewrite upd_Znth0 !cat_app Zlength_app. f_equal.
...
(*This means we have filled up the first i rows and the first j entries of row 
  i*)
      ...
      * (*Finally, we are not updating anything new. This is still annoying, 
        ...
        -- case : (x mod (2 * xh) <? j + 1) /Z.ltb_spec0 => 
          ...
          ++ have Hxj: x mod (2 * xh) = j by lia.
             have Hxcon: x = i * xh * 2 + j.
               by rewrite (Z_div_mod_eq_full x (2 * xh)); lia. by [].
...
Proof. rewrite mkseqZ_Zlength //; list_solve. Qed.
...
Proof.
  ...
  - rewrite pop_stats_Zlength => i Hi.
    rewrite mkseqZ_Znth // zseq_Znth //; [| list_solve].
...
Proof. move => Hx. rewrite mkseqZ_Znth // mkseqZ_Zlength //. list_solve. Qed. 
