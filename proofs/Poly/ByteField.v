...
Definition dlog (q : QF) :=  Ordinal (qlogp_lt p_notx q).
...
Proof.
move=> q_neq0. rewrite /dlog_alt.
by case: (q == 1) /eqP => /= q1; [rewrite q1 qX_order_card | apply: qlogp_qX].
Qed.
...
Lemma dlog_map_zero (q: {unit QF}) : qlogp p_notx (val q) = 0%N -> val q = 1.
Proof.
move=> log_eq.
...
- move => eq_ord. apply (f_equal val) in eq_ord. move: eq_ord => /=eq_dlog.
  have: (qlogp p_notx q2 <  (#|F| ^ (size p).-1).-1)%N by apply: qlogp_lt.
  by rewrite -eq_dlog ltnn.
- move => eq_ord. apply (f_equal val) in eq_ord. move: eq_ord => /=eq_dlog.
  have: (qlogp p_notx q1 <  (#|F| ^ (size p).-1).-1)%N by apply: qlogp_lt.
  by rewrite eq_dlog ltnn.
- move => [].
...
have i_small: (i < (#|F| ^ (size p).-1).-1)%N.
  ...
  by rewrite -ltnS prednK // (leq_trans (card_finField_gt0 p_notx)) ?leq_pred.
...
Definition qpoly_to_Z (q: @qpoly GF2 p256) : Z := poly_to_Z q.
...
Canonical byte_eQFype := EqType byte byte_eqMixin.
...
Proof. apply /beq_axiom. apply Byte.one_not_zero. Qed.

Definition byte_comringmixin :=
  ComRingMixin byte_mulA byte_mulC byte_mul1q byte_mulD byte_1not0.
...
Definition byte_unitringmixin :=
  UnitRingMixin byte_mulVr byte_mulrV byte_unitP byte_inv0id.
...
Proof. move => b Hnz. by apply byte_mulVr. Qed. 
...
(*Canonical byte_finFieldType := Eval hnf in [finFieldType of byte].*) 
...
Lemma ord_byte_bound (x: 'I_#|byte_finType|) : -1 < Z.of_nat x < Byte.modulus.
Proof.
  case: x => [n Hn] /=. rewrite card_byte in Hn. move: Hn => /ltP Hn. rep_lia.
Qed.

Definition ord_to_byte (x: 'I_#|byte_finType|) :=
  Byte.mkint _ (ord_byte_bound x).
...
Lemma byte_ord_bound (b : byte) :
  (Z.to_nat (Byte.unsigned b) < #|byte_finType|)%N.
Proof.
  rewrite card_byte. pose proof (Byte.unsigned_range b) as Hrange.
  apply /ltP. rep_lia.
Qed.

Definition byte_to_ord (b: byte) : 'I_#|_| := Ordinal (byte_ord_bound b).
...
Lemma byte_qpoly_size : #|byte_finType| = (#|bool_finType| ^ (size p256).-1)%N.
...
Definition byte_log_map (b : byte) : byte :=
  ord_to_byte (eq_ord (esym byte_qpoly_size) (dlog_alt p256_geq_2 
      ((byte_to_qpoly b) : Q256))).
  ...
  - apply val_inj.
    ...
    + rewrite -!Pdiv.IdomainMonic.modpE // mk_monicE; last first.
     ...
     rewrite Hlopoly -(@modp_small _ 
                        ('X * Z_to_poly (Byte_unsigned_nonneg (b:=b)) + p256) 
                        p256).
      * rewrite modpD modpp GRing.addr0 modp_mod.
...
Proof.
  ...
  have->:(Ziota 0 i ++ [:: (0 + i)%Z]) = (Ziota 0 i ++ [:: (0 + i)%Z])%list.
     by []. 
  rewrite /=.
    ...
    * split. 
      ...
      { move => b Hb. rewrite Hinv1; try lia. apply z_leq_n_1 in Hb.
        ...
        { rewrite upd_Znth_diff. apply IH; try by []. rep_lia.
          ...
          apply (congr1 (@dlog_alt _ _ p256_primitive p256_geq_2)) in Hbpow.
          move: Hbpow.
...
Proof.
  ...
  - move => i. rewrite Hsz2 => Hi. 
    ...
    * have->: Byte.modulus = Z.of_nat (256%N) by []. apply inj_lt.
      apply /ltP.
      case : (dlog_alt p256_geq_2 (byte_to_qpoly (Byte.repr i))) => [m Hm].
...
Proof. by rewrite -qX_order_card // card_bool. Qed.
...
Proof.
  ...
  have /ltP Hbound: (nat_of_ord (dlog_alt p256_geq_2 
  ...
  - remember (nat_of_ord (dlog_alt p256_geq_2 (byte_to_qpoly b))) as x.
  ...
  - f_equal. symmetry.
    ...
    have->: 
      (dlog_alt p256_geq_2 (q : Q256) +
        Z.to_nat (fec_n - 1 - Z.of_nat (dlog_alt p256_geq_2 (q : Q256))))%N
      = 255%N.
    { rewrite Z2Nat.inj_sub; try rep_lia. rewrite Nat2Z.id /=.
      set x := (dlog_alt _ _ : nat) in Hbound *.
...
Proof.
  ...
  have->:nat_of_ord (@dlog_alt _ _ p256_primitive p256_geq_2 0) = 0%N.
  apply /eqP.
...
Proof.
  ...
  (*
    have->: (@GRing.inv (qpoly_fieldType p256_irred)) (byte_to_qpoly b) =
     @qinv _ _ p256_irred (byte_to_qpoly b) by []. *)
...
Lemma ord_byte_simpl (x : 'I_#|byte_finType|) :
...
Proof.
have->:255%N=(#|bool_finFieldType| ^ (size p256).-1).-1.
   by rewrite size_p256 card_bool.
by apply: powX_eq_mod.
Qed.
...
Proof.
  ...
  have Hnz: forall (x y : byte), fec_n - 1 <
       Z.of_nat (dlog_alt p256_geq_2 ((byte_to_qpoly x) : Q256)) +
       Z.of_nat (dlog_alt p256_geq_2 ((byte_to_qpoly y : Q256))) ->
       x != 0. { move => x y Hbig'.
    case Hx: (x == 0) => [|//]. rewrite -byte_zero_qpoly_iff in Hx.
    apply (elimT eqP) in Hx. move: Hbig'.
    rewrite Hx. have->:nat_of_ord (dlog_alt p256_geq_2 (0 : Q256)) = 0%N.
  ...
  move: Hbig. set a := dlog_alt _ _ : nat; set b := dlog_alt _ _ : nat => Hbig.
