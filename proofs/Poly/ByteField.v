...
From mathcomp Require Import qpoly qfpoly.
...
Lemma Z_to_bits_last: forall z,
  0 <= z ->
  last (GRing.one bool) (Z_to_bits z) != 0%R.
Proof.
  move => z Hz.  rewrite /Z_to_bits. case : (Z.eq_dec z 0) => [Hz0 //= | Hz0 /=].
  have ->: GRing.one bool = Z.testbit z (Z.log2 z).
...
Lemma Z_bits_cancel: forall l, last (GRing.one bool) l != 0%R -> Z_to_bits (bits_to_Z l) = l.
Proof.
...
Definition poly_to_Z (p: {poly bool}) : Z :=
  bits_to_Z p.

Definition Z_to_poly (z: Z) (Hz: 0 <= z) : {poly bool} :=
...
Notation QF := {poly %/ p with primitive_mi p_prim}.

Lemma qlogp_lt' (q: QF) : (qlogp q < (#|F| ^ (size p).-1).-1)%N.
Proof.
  have:=(qlogp_lt q).
  rewrite card_primitive_qpoly // => Hlt.
Qed.

Definition dlog (q : QF) :=
  Ordinal (qlogp_lt' q).

Definition dlog_alt (q: QF) : 'I_(#|F| ^ (size p).-1) :=
  if q == 1 then Ordinal field_size_lt 
  else (widen_ord (ltnW field_size_lt) (dlog q)).
Import FinRing.Theory.

(*NOTE: copied from pull request, later removed so probably
  not necessary*)
Lemma card_finField_gt0 : (0 < (#|F|^(size p).-1).-1)%N.
Proof.
have cF : (1 < #|F|)%N by
  rewrite (cardD1 0) inE ltnS (cardD1 1) !inE GRing.oner_neq0.
rewrite -ltnS prednK ?expn_gt0 ?(leq_ltn_trans _ cF) //.
rewrite -[1%N](exp1n (size p).-1) ltn_exp2r //.
by rewrite  -ltnS prednK // (leq_ltn_trans _ p_notx).
Qed.

Lemma qX_order_card' : 'qX ^+ (#|F| ^ (size p).-1).-1 = 1 :> QF.
Proof.
have /primitive_polyP [_ Hd _] := p_prim.
rewrite dvdp_order in Hd.
have -> : 1 = val (fingroup.Notations.oneg _ : {unit QF}) by [].
by rewrite -(eqP Hd) val_unitX //= card_primitive_qpoly.
Qed.
...
case q_neq1: (q == 1) =>/=.
- by rewrite -lt0n card_finField_gt0.
- apply/eqP => log_zero.
  have := qlogp_qX p_notx q_neq0.
  rewrite log_zero GRing.expr0 => q_eq1.
...
move=> q_neq0. rewrite /dlog_alt.
by case: (q == 1) /eqP => /= q1; 
  [rewrite q1 -qX_order_card // card_primitive_qpoly |
   apply: qlogp_qX] .
Qed.

Lemma dlog_alt_zero : dlog_alt 0 = 0%N :> nat.
Proof.
rewrite /dlog_alt.
...
Lemma dlog_alt_eq0 (q: QF) : ((dlog_alt q) == 0%N :> nat) = (q == 0%R).
Proof.
case q_eq0: (q == 0).
...
have: (0 < 0)%N. rewrite -{2}field0. by apply card_finField_gt0.
by rewrite ltnn.
Qed.

Lemma dlog_map_zero (q: {unit QF}) : qlogp (val q) = 0%N -> val q = 1.
Proof.
move=> log_eq.
have q_neq0 : val q != 0 by rewrite -GRing.unitfE //; apply/valP.
by have := qlogp_qX p_notx q_neq0; rewrite log_eq GRing.expr0.
Qed.
...
- move => eq_ord. apply (f_equal val) in eq_ord. move: eq_ord => /=eq_dlog.
  have: (qlogp q2 <  (#|F| ^ (size p).-1).-1)%N by apply: qlogp_lt'.
  by rewrite -eq_dlog ltnn.
- move => eq_ord. apply (f_equal val) in eq_ord. move: eq_ord => /=eq_dlog.
  have: (qlogp q1 <  (#|F| ^ (size p).-1).-1)%N by apply: qlogp_lt'.
  by rewrite eq_dlog ltnn.
- move => [].
  case: (q1 =P 0) => [-> /eqP|/eqP q1_neq0].
    by rewrite qlogp0 // eq_sym qlogp_eq0 // => /orP[] /eqP.
  case: (q2 =P 0) => [-> /eqP|/eqP q2_neq0 Hq].
    by rewrite qlogp0 // qlogp_eq0 // => /orP[] /eqP.
  by rewrite -[q1]qlogp_qX // Hq qlogp_qX.
Qed.

(*We also define the corresponding power map*)
Definition qpow_map_alt (i: 'I_(#|F|^((size p).-1))) : QF := 
  if (i == 0%N :> nat) then 0 else 'qX ^+ i.

Definition qpow_map_full (i: 'I_(#|F|^((size p).-1))): QF := 'qX ^+ i.

Lemma qpow_map_full_equiv (i : 'I_ _) :
  i != 0%N :> nat -> qpow_map_full i = qpow_map_alt i.
Proof.
rewrite /qpow_map_alt /qpow_map_full. by case : (nat_of_ord i == 0%N).
Qed.

Lemma qpow_map_full_inv (i : 'I_ _) :
  i != 0%N :> nat -> dlog_alt (qpow_map_full i) = i.
Proof.
...
case: (i == Ordinal field_size_lt) /eqP => i_big.
  by rewrite i_big /= qX_order_card' // eqxx.
(*Now we have an ordinal in the correct range*)
have i_small: (i < (#|F| ^ (size p).-1).-1)%N.
  rewrite ltn_neqAle. apply /andP; split.
    apply /eqP => i_eq. apply i_big. by apply ord_inj.
  by rewrite -ltnS prednK // (leq_trans (card_finField_gt0)) ?leq_pred.
rewrite ifN; last first.
  have i_gt0 : (0 < i)%N by rewrite lt0n.
  apply/eqP => /(qX_order_dvd p_notx) /(dvdn_leq i_gt0).
  rewrite card_primitive_qpoly //.
  by rewrite leqNgt i_small.
apply/val_inj/qX_expK =>//.
by rewrite card_primitive_qpoly.
Qed.

Lemma qpow_map_full_neq0 i : qpow_map_full i != 0.
Proof.
rewrite /qpow_map_full. rewrite GRing.expf_neq0 //.
by rewrite -['qX]GRing.expr1 qX_exp_neq0.
Qed.
...
Lemma size_p256: size p256 = 9%N.
Proof. by []. Qed.

(*Now we can convert between bytes and qpolys*)
Definition qpoly_to_Z (q: @qpoly bool p256) : Z := poly_to_Z q.

Lemma qpoly_to_Z_bound q : -1 < qpoly_to_Z q < Byte.modulus.
Proof.
  case: q => [p Hp].
  rewrite /qpoly_to_Z /=. pose proof (poly_to_Z_size p) as [Hlo Hhi].
  move: Hp; rewrite mk_monicE; last by apply: p256_irred.
  rewrite size_p256 /= qualifE => /leP/inj_le /= Hp.
  have->: Byte.modulus = 256 by []. split; try lia.
  have: 2 ^ Z.of_nat (size p) - 1 <= 2 ^ 8 - 1.
    rewrite -Z.sub_le_mono_r.
    by apply Z.pow_le_mono_r.
  have->: (2 ^ 8 - 1)%Z = 255 by []. move => Hhi'. 
  have: poly_to_Z p <= 255 by apply (Z.le_trans _ _ _ Hhi Hhi'). lia.
Qed.

Lemma qpoly_to_Z_0  (q : Q256) : qpoly_to_Z q = 0 <-> q = 0%R.
Proof.
  case: q => [[p Hlast] Hsz]. rewrite /qpoly_to_Z /poly_to_Z. split.
  - move => Hb. apply val_inj. apply val_inj. rewrite /=. rewrite polyseq0. 
    by apply (bits_to_Z_zero Hlast).
  - move ->. by rewrite polyseq0.
Qed.

Definition qpoly_to_byte (q: Q256) : byte := Byte.mkint _ (qpoly_to_Z_bound q).

Definition byte_to_poly (b: byte) :  {poly bool} :=
  Z_to_poly (@Byte_unsigned_nonneg b).

Lemma byte_to_poly_range b : (size (byte_to_poly b) < size p256)%N.
Proof.
  rewrite /byte_to_poly Z_to_poly_size size_p256.
  case : (Z.eq_dec (Byte.unsigned b) 0) => [//= | /= Hb].
  rewrite -addn1 -(addn1 8) leq_add2r. apply /ltP.
  pose proof (byte_log2_range b). lia.
Qed.

Definition byte_to_qpoly (b: byte) : Q256 := in_qpoly _ (byte_to_poly b).

Lemma byte_to_qpolyE (b: byte) :
 byte_to_qpoly b = byte_to_poly b :> {poly bool}.
Proof.
rewrite in_qpoly_small // mk_monicE ?byte_to_poly_range //.
by apply/primitive_mi/p256_primitive.
Qed.

(*Now, we have to show that these maps form a bijection, and *)
(*then we can define the ring and field operations*)
Lemma byte_qpoly_cancel: cancel byte_to_qpoly qpoly_to_byte.
Proof.
  move => b. apply byte_unsigned_inj.
  rewrite /qpoly_to_byte /qpoly_to_Z /Byte.unsigned /Byte.intval byte_to_qpolyE.
  apply bits_Z_cancel =>> /=.
  case: b. rep_lia.
Qed.
...
Proof.
  move => s. apply val_inj.
  rewrite [LHS]byte_to_qpolyE.
  apply val_inj. apply Z_bits_cancel.
  by case : s => [[p Hp] Hsz /=].
Qed.

Lemma byte_qpoly_bij: bijective byte_to_qpoly.
Proof. apply (Bijective byte_qpoly_cancel). apply qpoly_byte_cancel. Qed.

Lemma byte_to_qpoly_inj: injective byte_to_qpoly.
Proof. apply bij_inj. apply byte_qpoly_bij. Qed.
...
HB.instance Definition _ :=
  Countable.copy byte (can_type byte_qpoly_cancel).

HB.instance Definition _ :=
  isFinite.Build byte
    (@pcan_enumP byte _ _ _ (can_pcan byte_qpoly_cancel)).

Lemma card_byte: #|(byte: finType)| = 256%N.
Proof.
  rewrite (bijective_card byte_qpoly_bij) card_qpoly card_bool.
  rewrite mk_monicE ?size_p256 //.
  apply: primitive_mi.
...
Lemma xor_qpoly (b1 b2 : byte) :
  byte_to_qpoly (Byte.xor b1 b2) = (byte_to_qpoly b1 + byte_to_qpoly b2).
Proof.
  apply val_inj. rewrite /byte_to_poly /Byte.xor.
  rewrite [LHS]byte_to_qpolyE /byte_to_poly.
  rewrite (@Z_to_poly_eq _ (Z.lxor (Byte.unsigned b1) (Byte.unsigned b2))).
  - apply Z_lxor_nonneg'; rep_lia.
  - move => Hxor.
    rewrite [RHS]GRing.linearD ![_ (byte_to_qpoly _)]byte_to_qpolyE.
    by apply: xor_poly.
  - rewrite Byte.unsigned_repr //. split. apply Z_lxor_nonneg'; rep_lia.
    pose proof (Z.log2_lxor (Byte.unsigned b1) (Byte.unsigned b2)).
    have: Z.max (Z.log2 (Byte.unsigned b1)) (Z.log2 (Byte.unsigned b2)) <=
          Z.log2 255. {
      apply Z.max_lub; apply Z.log2_le_mono; rep_lia. } rewrite /= => Hup.
    have: Z.log2 (Z.lxor (Byte.unsigned b1) (Byte.unsigned b2)) < 
          Z.log2 (Byte.modulus). rewrite /=. rep_lia.
    move => Hloglt. apply Z.log2_lt_cancel in Hloglt. rep_lia.
Qed.

Lemma byte_zero_qpoly : byte_to_qpoly Byte.zero = 0.
Proof.
  apply val_inj.
  rewrite [LHS]byte_to_qpolyE.
...
Lemma byte_zero_qpoly_iff b : (byte_to_qpoly b == 0) = (b == Byte.zero).
Proof.
  suff <-: byte_to_qpoly Byte.zero = 0  by rewrite (inj_eq byte_to_qpoly_inj).
  apply/val_inj; rewrite [LHS]byte_to_qpolyE /=.
  apply/val_inj => //=.
  rewrite Byte.unsigned_repr_eq Z.mod_0_l //=.
  by rewrite polyseq0.
Qed.

Lemma byte_one_qpoly: byte_to_qpoly Byte.one = 1.
Proof.
  apply/val_inj; rewrite [LHS]byte_to_qpolyE.
...
(*opp is easy*)
Lemma opp_qpoly (b1: byte) : byte_to_qpoly b1 = - (byte_to_qpoly b1).
Proof.
  apply val_inj; 
  rewrite [LHS]byte_to_qpolyE [RHS]GRing.linearN [X in _ = - X]byte_to_qpolyE.
...
Definition byte_mul (b1 b2: byte) : byte :=
  qpoly_to_byte (byte_to_qpoly b1 * byte_to_qpoly b2).

Definition byte_inv (b: byte) : byte := qpoly_to_byte ((byte_to_qpoly b)^-1).

(*This proves (implictly) that these rings are isomorphic. Since we need the*)
(*canonical instances, we prove all the conditions manually, but they all*)
(**follow from the above isomorphism*)

Lemma baddA : associative Byte.xor.
Proof. move => b1 b2 b3. by rewrite Byte.xor_assoc. Qed.

Lemma baddC : commutative Byte.xor.
Proof. move => b1 b2. apply Byte.xor_commut. Qed.

Lemma baddFb : left_id Byte.zero Byte.xor.
Proof. move => b. apply Byte.xor_zero_l. Qed.

Lemma baddbb : left_inverse Byte.zero id Byte.xor.
Proof. move => b. apply Byte.xor_idem. Qed.

Lemma byte_mulA : associative byte_mul.
Proof.
  move => b1 b2 b3. rewrite /byte_mul. f_equal.
  by rewrite !qpoly_byte_cancel GRing.mulrA.
Qed.
...
Lemma byte_mulq1: right_id Byte.one byte_mul.
Proof.
  move => b. by rewrite byte_mulC byte_mul1q. 
Qed.
...
Lemma byte_mulDr: right_distributive byte_mul Byte.xor.
Proof.
  move => b1 b2 b3. rewrite /byte_mul. apply byte_to_qpoly_inj. 
  by rewrite !xor_qpoly !qpoly_byte_cancel GRing.mulrDr.
Qed.

Lemma byte_1not0 : Byte.one != Byte.zero.
Proof. apply /eqP. apply Byte.one_not_zero. Qed. 

HB.instance Definition _ := GRing.isRing.Build byte
baddA baddC baddFb baddbb 
byte_mulA  byte_mul1q byte_mulq1 byte_mulD byte_mulDr byte_1not0.
HB.instance Definition _ := GRing.Ring_hasCommutativeMul.Build 
  byte byte_mulC.

Definition bunit : pred byte := [pred x | x != Byte.zero].

Lemma bunit_qunit b : bunit b = (byte_to_qpoly b != 0).
Proof. by rewrite /bunit byte_zero_qpoly_iff. Qed.
...
Proof.
  ...
  - rewrite GRing.unitfE. have: b != Byte.zero by [].
    by rewrite byte_zero_qpoly_iff.
Qed.

Lemma byte_mulrV: {in bunit, right_inverse Byte.one byte_inv byte_mul}.
Proof. move => b Hin. rewrite byte_mulC. by apply byte_mulVr. Qed.

Lemma byte_unitP (x y : byte) : (y * x) = 1 /\ (x * y) = 1 -> bunit x.
Proof.
  have->: (x * y) = byte_mul x y by [].
  have->: (y * x) = byte_mul y x by []. rewrite /byte_mul => [[Hun1 Hun2]].
  have Hun: byte_to_qpoly y * byte_to_qpoly x = 1 /\
        byte_to_qpoly x * byte_to_qpoly y = 1. {
  split.
  - apply (congr1 byte_to_qpoly) in Hun1. move: Hun1; rewrite qpoly_byte_cancel.
    move ->. by apply byte_one_qpoly.
  - apply (congr1 byte_to_qpoly) in Hun2. move: Hun2; rewrite qpoly_byte_cancel.
    move ->. by apply byte_one_qpoly. }
  rewrite bunit_qunit.
  apply/eqP=> H0; case: Hun => /eqP; rewrite H0 GRing.mulr0.
  by rewrite eq_sym GRing.oner_eq0.
Qed.

Lemma byte_inv0id : {in [predC bunit], byte_inv =1 id}.
Proof.
  move => b Hun. have Hz: ~~ (b != 0) by []. move: Hz; rewrite negbK => /eqP Hz.
  subst.
...
HB.instance Definition _ := GRing.Ring_hasMulInverse.Build byte
byte_mulVr byte_mulrV byte_unitP byte_inv0id.

Lemma byte_mulf_eq0 : forall x y : byte, (x * y) = 0 -> (x == 0) || (y == 0).
Proof.
  move => x y. have->: (x * y) = byte_mul x y by []. have->:0 = Byte.zero by [].
  rewrite /byte_mul => Hq.
  apply (congr1 byte_to_qpoly) in Hq.
  move: Hq; rewrite qpoly_byte_cancel byte_zero_qpoly => /eqP Hz.
  rewrite GRing.mulf_eq0 in Hz. by rewrite -!byte_zero_qpoly_iff.
Qed.

HB.instance Definition _ := GRing.ComUnitRing_isIntegral.Build byte
byte_mulf_eq0.

Lemma byte_mulVf : GRing.field_axiom byte.
Proof.
  by [].
Qed.

Lemma byte_inv0 : byte_inv 0%R = 0%R.
Proof. by apply byte_inv0id. Qed. 

HB.instance Definition _ := GRing.UnitRing_isField.Build byte
byte_mulVf.

Lemma F_char_2 : 2%N \in [char byte]%R.
Proof. rewrite /GRing.char //= /in_mem /= GRing.mulr2n /=. Qed.
...
Lemma ord_byte_bound (x: 'I_#|(byte: finType)|): -1 < Z.of_nat x < Byte.modulus.
Proof.
  case: x => [n Hn] /=. rewrite card_byte in Hn. move: Hn => /ltP Hn. rep_lia.
Qed.

Definition ord_to_byte (x: 'I_#|(byte: finType)|) := Byte.mkint _ (ord_byte_bound x).

Lemma ord_to_byte_zero (x : 'I_ _) : x = 0%N :> nat -> ord_to_byte x = Byte.zero.
Proof.
  move => Hx. apply byte_unsigned_inj. by rewrite /= Hx Byte.unsigned_zero.
Qed.

Lemma byte_ord_bound (b : byte) :
  (Z.to_nat (Byte.unsigned b) < #|(byte : finType)|)%N.
Proof.
  rewrite card_byte. pose proof (Byte.unsigned_range b) as Hrange.
  apply /ltP. rep_lia.
Qed.

Definition byte_to_ord (b: byte) : 'I_#|(byte: finType)| := Ordinal (byte_ord_bound b).

(*Now, we can define the power and inverse power (log) maps*)

Lemma p256_geq_2 : (2 < size p256)%N.
Proof. by rewrite size_p256. Qed.

Lemma byte_qpoly_size : #|(byte: finType)| = (#|(bool : finType)| ^ (size p256).-1)%N.
Proof. by rewrite card_bool size_p256 card_byte. Qed.

(*We don't want to use Byte.modulus directly because it will simplify*)

Definition byte_pow_map (b : byte) : byte :=
  qpoly_to_byte
    (qpow_map_full p256_primitive (eq_ord byte_qpoly_size (byte_to_ord b))).

Definition byte_log_map (b: byte) : byte :=
  ord_to_byte (eq_ord (esym byte_qpoly_size) (dlog_alt (byte_to_qpoly b))).

Definition byte_pows : seq byte := mkseqByte byte_pow_map fec_n.

Definition byte_logs : seq byte := mkseqByte byte_log_map fec_n.

Definition byte_invs: seq byte := mkseqByte byte_inv fec_n.

(*So we don't need to unfold the definitions ever*)
Lemma byte_pows_Zlength: Zlength byte_pows = Byte.modulus.
Proof. by rewrite /byte_pows mkseqByte_Zlength; rep_lia. Qed.

Lemma byte_pows_Znth i :
  0 <= i < Byte.modulus -> Znth i byte_pows = byte_pow_map (Byte.repr i).
Proof. move => Hi. by apply mkseqByte_Znth_Z; rep_lia. Qed.

Lemma byte_logs_Zlength: Zlength byte_logs = Byte.modulus.
Proof. by rewrite /byte_logs mkseqByte_Zlength; rep_lia. Qed.

Lemma byte_logs_Znth i :
  0 <= i < Byte.modulus -> Znth i byte_logs = byte_log_map (Byte.repr i).
Proof. move => Hi. by apply mkseqByte_Znth_Z; rep_lia. Qed.

Lemma byte_invs_Zlength : Zlength byte_invs = Byte.modulus.
Proof. by rewrite /byte_invs mkseqByte_Zlength; rep_lia. Qed.

Lemma byte_invs_Znth i : 
  0 <= i < Byte.modulus -> Znth i byte_invs = byte_inv (Byte.repr i).
Proof. move => Hi. by apply mkseqByte_Znth_Z; rep_lia. Qed.
...
(*The element "x"*)
Definition bx := qpoly_to_byte 'qX.

Lemma byte_exp_unfold (q : Q256) n :
  qpoly_to_byte (q ^+ n) = (qpoly_to_byte q) ^+ n.
Proof.
  elim : n => [//= | n' /= IH].
  - rewrite !GRing.expr0. apply byte_to_qpoly_inj.
    by rewrite byte_one_qpoly qpoly_byte_cancel.
  - rewrite !GRing.exprS -IH.
   apply byte_to_qpoly_inj. rewrite qpoly_byte_cancel.
    by rewrite /byte_mul !qpoly_byte_cancel.
Qed.

(* Describes clearly what byte_pows is*)
Lemma byte_pows_elts i :
  0 <= i < Byte.modulus -> Znth i byte_pows = bx ^+ (Z.to_nat i).
Proof.
  move => Hi. rewrite byte_pows_Znth // /byte_pow_map /bx /qpow_map_full
                      /= Byte.unsigned_repr.
  2: rep_lia. apply byte_exp_unfold.
Qed.

(*Describes clearly what byte_logs is*)
Lemma byte_logs_elts i :
  0 < i < Byte.modulus -> 
  bx ^+ Z.to_nat (Byte.unsigned (Znth i byte_logs)) = Byte.repr i.
Proof.
  move => Hi. rewrite byte_logs_Znth // /byte_log_map; try lia. 
  rewrite /bx/= Nat2Z.id -byte_exp_unfold dlog_alt_correct //.
  - apply byte_qpoly_cancel.
  - apply negbT. rewrite byte_zero_qpoly_iff. apply /eqP. move => Hiz.
    apply (congr1 Byte.unsigned) in Hiz.
    move: Hiz. rewrite Byte.unsigned_zero Byte.unsigned_repr; rep_lia.
Qed.

Lemma byte_invs_elts i :
  0 <= i < Byte.modulus -> Znth i byte_invs = (Byte.repr i)^-1.
Proof. move => Hi. by rewrite byte_invs_Znth. Qed.
...
(*We will model the functions that populate the lookup tables here, so the*)
(*VST proof does not need to directly reason about any field properties*)

Lemma modulus_nonneg : 0 <= modulus.
Proof. rep_lia. Qed.

Lemma modulus_val_poly : Z_to_poly modulus_nonneg = p256.
Proof. apply val_inj. by rewrite/= modulus_eq. Qed. 
...
Lemma Z_shiftl_poly z (Hz : 0 <= z) (Hshift : 0 <= Z.shiftl z 1) :
  Z_to_poly Hshift = 'X * (Z_to_poly Hz).
Proof.
  rewrite -polyP => i. rewrite coefXM /Z_to_poly /polyseq !Z_to_bits_nth //.
...
Lemma byte_mulX_correct b : byte_to_qpoly (byte_mulX b) = 'qX * byte_to_qpoly b.
Proof.
  rewrite /byte_mulX. 
  case : (Z_lt_ge_dec (Z.shiftl (Byte.unsigned b) 1) Byte.modulus) =>
    [Hsmall |Hbig];
  rewrite /proj_sumbool.
  - apply: val_inj.
    rewrite [LHS]byte_to_qpolyE.
    rewrite /byte_to_poly.
    rewrite (@Z_to_poly_eq _ (Z.shiftl (Byte.unsigned b) 1)).
    + apply Z_shiftl_nonneg'. rep_lia.
    + move => Hb1. rewrite Z_shiftl_poly. apply Byte_unsigned_nonneg.
      move => Hb0. rewrite  [RHS]Pdiv.CommonRing.rmodp_small.
        congr (_ * _); first by rewrite qpolyXE.
        rewrite [RHS]byte_to_qpolyE.
        by apply Z_to_poly_eq.
      have [{}Hb0 | {}Hb0]: Byte.unsigned b <> 0%Z \/ Byte.unsigned b = 0%Z.
        by lia.
      * rewrite size_mul.
        -- rewrite qpolyXE // size_polyX.
           rewrite [in X in (_ < X)%N]mk_monicE; last first.
             by apply/primitive_mi/p256_primitive.
           rewrite byte_to_qpolyE /= Z_to_poly_size; try rep_lia.
          case : (Z.eq_dec (Byte.unsigned b) 0) => [// | {}Hb0 /=]; try lia.
          have {}Hsmall: Z.shiftl (Byte.unsigned b) 1 <= 255 by rep_lia.
          have Hlog: Z.log2 (Z.shiftl (Byte.unsigned b) 1) = 
                    (Z.log2 (Byte.unsigned b) + 1)%Z. {
            rewrite Z.log2_shiftl; rep_lia. } apply /ltP.
          apply Z.log2_le_mono in Hsmall.
           move: Hsmall; rewrite Hlog /=. lia.
        -- by rewrite qpolyXE // polyX_eq0.
        --  rewrite byte_to_qpolyE.
            apply /eqP. by rewrite Z_to_poly_zero.
    * rewrite byte_to_qpolyE /byte_to_poly.
      have->: Z_to_poly (Byte_unsigned_nonneg (b:=b)) = 0%R.
      by rewrite Z_to_poly_zero.
        by rewrite GRing.mulr0 size_poly0 size_p256.
    + rewrite Byte.unsigned_repr //. split; try rep_lia. apply Z_shiftl_nonneg'.
      rep_lia.
  - apply val_inj.
    have [Hlo Hhi]: 0 <= Z.lxor (Z.shiftl (Byte.unsigned b) 1) modulus 
                      <= Byte.max_unsigned. {
      move: Hbig. rewrite Zbits.Zshiftl_mul_two_p; try lia.
      have->: two_p 1 = 2%Z by [].
      rewrite Z.mul_comm => Hbig. pose proof (xor_modulus_bound Hbig).
      rep_lia. }
    have Hbyte: 
      Byte.unsigned (Byte.repr (Z.lxor (Z.shiftl (Byte.unsigned b) 1) modulus))
       =
      Z.lxor (Z.shiftl (Byte.unsigned b) 1) modulus.
      by rewrite Byte.unsigned_repr.
    have Hlopoly: Z_to_poly Hlo =
                  (('X * Z_to_poly (Byte_unsigned_nonneg (b:=b)) + p256)). {
      rewrite xor_poly. apply Z_shiftl_nonneg'; rep_lia. rep_lia.
      move => Hshl Hp256. rewrite Z_shiftl_poly. rep_lia. move => Hb0.
      f_equal. f_equal. by apply Z_to_poly_eq.
      rewrite -modulus_val_poly. by apply Z_to_poly_eq. }
    rewrite /= /byte_to_poly /= 
            (@Z_to_poly_eq _  (Z.lxor (Z.shiftl (Byte.unsigned b) 1) modulus)).
    + rewrite -!Pdiv.IdomainMonic.modpE // mk_monicE; last first.
        by apply/primitive_mi/p256_primitive.
     rewrite Hlopoly -(@modp_small _ 
                        ('X * Z_to_poly (Byte_unsigned_nonneg (b:=b)) + p256) 
                        p256).
      * rewrite modpD modpp GRing.addr0 modp_id.
        by rewrite modp_mul [in RHS]GRing.mulrC modp_mul [in RHS]GRing.mulrC.
      * rewrite -Hlopoly Z_to_poly_size size_p256.
        case : (Z.eq_dec (Z.lxor (Z.shiftl (Byte.unsigned b) 1) modulus) 0) =>
            [// | /= Hx0].
...
Definition populate_pows_logs_aux (l: seq Z) 
            (base : (seq byte) * (seq byte)) : (seq byte) * (seq byte) :=
  fold_left (fun (acc: seq byte * seq byte) z => 
    ...
    let newLogs := upd_Znth (Byte.unsigned (Znth z newPows)) logs 
                            (Byte.repr z) in
...
Lemma populate_pows_logs_iota_aux_plus_1: forall i base,
  ...
  if Z.eq_dec i 0 then
    (upd_Znth i (populate_pows_logs_iota_aux i base).1 Byte.one, 
...
Lemma populate_pows_logs_length1 i :
   Zlength (fst (populate_pows_logs i)) = Byte.modulus.
Proof.
  rewrite /populate_pows_logs populate_pows_logs_aux_length1 /=.
  apply zseq_Zlength. rep_lia.
Qed.

Lemma populate_pows_logs_aux_length2 l b :
...
Lemma populate_pows_logs_length2 i :
   Zlength (snd (populate_pows_logs i)) = Byte.modulus.
Proof.
  rewrite /populate_pows_logs populate_pows_logs_aux_length2 /=.
  apply zseq_Zlength. rep_lia.
Qed.

Lemma populate_pows_logs_plus_1 (i: Z) :
  0 <= i ->
  populate_pows_logs (i+1) =
  if Z.eq_dec i 0 then 
    (upd_Znth i (populate_pows_logs i).1 Byte.one, (populate_pows_logs i).2) 
  else
  (upd_Znth i (populate_pows_logs i).1 
    (byte_mulX (Znth (i-1) (populate_pows_logs i).1)),
    upd_Znth (Byte.unsigned (Znth i (populate_pows_logs (i+1)).1))
      (populate_pows_logs i).2 (Byte.repr i)).
Proof.
  move => Hi. rewrite {1}/populate_pows_logs.
  by rewrite populate_pows_logs_iota_aux_plus_1.
Qed.

(*Why do we need these? Why doesn't lia work directly?*)
Lemma nat_leq_1 n : Z.of_nat n < 1 -> n = 0%N.
Proof. lia. Qed.

Lemma z_leq_n_1 z n : z < n + 1 -> z < n \/ z = n.
Proof. lia. Qed.

Lemma p256_primitive': primitive_poly p256.
Proof. apply p256_primitive. Qed.

(*Now, we need to prove that this is actually correct*)

(*The "loop invariant". The first part is not hard, since we fill up each     *)
(*power in order. But we fill in the logs in an unknown order, so we need to  *)
(*make a claim about all powers whose log is below the current loop value,    *)
(*then later prove that we fill the list in correctly because each individual *)
(*element is filled in correctly                                              *)
Definition populate_pows_logs_invar (l : seq byte * seq byte) (i : Z) :=
  (forall z, 0 <= z < i -> Znth z l.1 = byte_pow_map (Byte.repr z)) /\ 
  Znth 0 l.2 = Byte.zero /\
  (forall (b : byte), Byte.unsigned (byte_log_map b) < i -> 
    Znth (Byte.unsigned b) l.2 = byte_log_map b).

Lemma populate_pows_logs_correct_ind (i : Z) base :
  0 <= i <= Byte.modulus ->
...
Proof.
  move => Hi. have Hinat: i = Z.of_nat (Z.to_nat i) by lia. move: Hinat Hi ->.
  move : base. elim : (Z.to_nat i) => [//= base Hi | n IH base].
  ...
  - have->: Z.of_nat n.+1 = (Z.of_nat n + 1) %Z by lia.
    move => Hn1 Hlen1 Hlen2 Hbase2.
    move: IH; rewrite /populate_pows_logs_invar => IH.
    (*We need the first one in the second one*)
    have Hinv1: (forall z : Z,
      0 <= z < Z.of_nat n + 1 ->
      Znth z (populate_pows_logs_iota_aux (Z.of_nat n + 1) base).1 = 
      byte_pow_map (Byte.repr z)). {
      move => z Hz. rewrite populate_pows_logs_iota_aux_plus_1; try lia.
      case : (Z.eq_dec (Z.of_nat n) 0) => [Hn0 /= | Hn0 /=].
      * have->: z = 0%Z by lia. rewrite Hn0.
        rewrite /populate_pows_logs_iota_aux /=.
        rewrite upd_Znth_same. have->:Byte.repr 0 = Byte.zero by [].
        by rewrite byte_pow_map_zero. lia.
      * have [Hbef | Hcurr]: (0 <= z < Z.of_nat n)%Z \/ (z = Z.of_nat n)%Z.
          by lia.
        -- rewrite upd_Znth_diff; try lia. apply IH; try rep_lia. by [].  
           rewrite populate_pows_logs_aux_length1. lia. 
           rewrite populate_pows_logs_aux_length1. lia. 
        -- rewrite Hcurr upd_Znth_same; last first.
             rewrite populate_pows_logs_aux_length1. lia. 
           have Hprev:
             (Znth (z - 1) (populate_pows_logs_iota_aux (Z.of_nat n) base).1)
              = byte_pow_map (Byte.repr (z-1)).
           apply IH; try lia. by []. rewrite Hcurr in Hprev.
           rewrite Hprev. apply byte_to_qpoly_inj.
           rewrite byte_mulX_correct.
           rewrite /byte_pow_map !qpoly_byte_cancel /qpow_map_full.
           rewrite -{1}(GRing.expr1 'qX) -GRing.exprD. f_equal. rewrite /=.
           rewrite !Byte.unsigned_repr; try rep_lia. 
           have->: (1 + Z.to_nat (Z.of_nat n - 1))%N = 
                   (1 + Z.to_nat (Z.of_nat n - 1))%coq_nat by []. lia. }
    split => [//| ].  rewrite populate_pows_logs_iota_aux_plus_1; try lia.
    case : (Z.eq_dec (Z.of_nat n) 0) => [Hn0 /= | Hn0 /=].
    * rewrite Hn0 /populate_pows_logs_iota_aux /= /byte_log_map.
      split => [//|b Hb]. 
      move: Hb => Hpow. apply nat_leq_1 in Hpow.
      rewrite ord_to_byte_zero //.
      apply (introT eqP) in Hpow. move: Hpow. rewrite dlog_alt_eq0 //; last first.
    ...
    * split. 
      { rewrite upd_Znth_diff //. apply IH; try rep_lia. by [].
        ...
        rewrite Hinv1; try lia. rewrite /byte_pow_map /= => Hzero.
        apply esym in Hzero; move: Hzero.
        rewrite qpoly_to_Z_0 => Hpow.
        set u := eq_ord _ _ in Hpow.
        by have /eqP[] := qpow_map_full_neq0 p256_primitive isT u.
      }
      { move => b Hb. rewrite Hinv1; try lia. apply z_leq_n_1 in Hb.
        case : Hb => [Hbef | Hcurr].
        { rewrite upd_Znth_diff. apply IH; try by []. rep_lia.
          rewrite populate_pows_logs_aux_length2; rep_lia.
          rewrite populate_pows_logs_aux_length2; rep_lia.
          move: Hbef. rewrite -Nat2Z.inj_lt /byte_pow_map => Hpow Hbpow. 
          apply byte_unsigned_inj in Hbpow.
          apply (congr1 byte_to_qpoly) in Hbpow. 
          apply (congr1 (@dlog_alt _ _ p256_primitive)) in Hbpow.
          move: Hbpow.
        ...
        { have Hbpow: (byte_pow_map (Byte.repr (Z.of_nat n))) = b. {
            apply Nat2Z.inj in Hcurr. rewrite /byte_pow_map.
            apply byte_to_qpoly_inj.
            rewrite qpoly_byte_cancel. 
            apply (@dlog_alt_inj _ _ (p256_primitive) p256_geq_2).
            rewrite qpow_map_full_inv //=. apply ord_inj => /=.
            rewrite Hcurr /=. by rewrite Byte.unsigned_repr; rep_lia.
          ...
          rewrite /byte_log_map /byte_pow_map. move <-.
          rewrite qpoly_byte_cancel qpow_map_full_inv //=.
...
Proof.
  rewrite /populate_pows_logs. 
  remember (zseq Byte.modulus Byte.zero, zseq Byte.modulus Byte.zero) as base.
  ...
  pose proof (populate_pows_logs_correct_ind Hmod Hb1 Hb2 Hb0) as Hinvar.
  move: Hinvar.
  case Hpop : (populate_pows_logs_iota_aux Byte.modulus base) => [pows logs].
  have Hsz1: Zlength pows = Byte.modulus. { 
    have->: pows = (populate_pows_logs_iota_aux Byte.modulus base).1.
      by rewrite Hpop.
    by rewrite populate_pows_logs_aux_length1. }
  have Hsz2: Zlength logs = Byte.modulus. {
    have->: logs = (populate_pows_logs_iota_aux Byte.modulus base).2.
      by rewrite Hpop.
  ...
  - move => i. rewrite Hsz1 => Hi. rewrite Hfst // /byte_pows mkseqByte_Znth_Z.
    by []. rep_lia. }
  have->: logs = byte_logs. { apply Znth_eq_ext.
  - rewrite /byte_pows mkseqByte_Zlength; rep_lia.
  - move => i. rewrite Hsz2 => Hi. 
    have Hibyte: i = Byte.unsigned (Byte.repr i).
      by rewrite Byte.unsigned_repr; rep_lia.
    ...
    * have->: Byte.modulus = Z.of_nat (256%N) by []. apply inj_lt.
      apply /ltP.
      case : (dlog_alt (byte_to_qpoly (Byte.repr i))) => [m Hm].
      rewrite /=. move: Hm. by rewrite card_bool size_p256.
  }
  by [].
Qed.

(*Similarly, we will have a functional version of the VST code for generating *)
(*the inverse table                                                           *)

Definition populate_invs_aux i base :=
  fold_left (fun acc z => 
    let inv := byte_pow_map 
      (Byte.repr (fec_n - 1 - Byte.unsigned (byte_log_map (Byte.repr z)))) in
    upd_Znth (Byte.unsigned inv) acc (Byte.repr z)) (Ziota 0 i) base.

Definition populate_invs i := populate_invs_aux i (zseq Byte.modulus Byte.zero).

Lemma populate_invs_0: populate_invs 0 = zseq Byte.modulus Byte.zero.
Proof. by rewrite /populate_invs. Qed.

Lemma populate_invs_aux_plus_1 i base :
  0 <= i ->
  populate_invs_aux (i+1) base =
  upd_Znth (Byte.unsigned
     (byte_pow_map (Byte.repr
                     (fec_n - 1 - Byte.unsigned (byte_log_map (Byte.repr i))))))
    (populate_invs_aux i base) (Byte.repr i).
Proof.
  move => iHi. rewrite /populate_invs_aux Ziota_plus_1; try lia.
  by rewrite fold_left_app.
Qed.

Lemma populate_invs_plus_1 i :
    0 <= i ->
  populate_invs (i+1) =
  upd_Znth (Byte.unsigned (byte_pow_map
      (Byte.repr (fec_n - 1 - Byte.unsigned (byte_log_map (Byte.repr i))))))
    (populate_invs i) (Byte.repr i).
Proof. move => Hi. by apply populate_invs_aux_plus_1. Qed.


Lemma populate_invs_aux_length i base :
  Zlength (populate_invs_aux i base) = Zlength base.
Proof.
  rewrite /populate_invs_aux. 
  elim : (Ziota 0 i) base => [base //= | h t /= IH base].
  by rewrite IH Zlength_upd_Znth.
Qed.

Lemma populate_invs_length i : Zlength (populate_invs i) = Byte.modulus.
Proof. by rewrite populate_invs_aux_length zseq_Zlength. Qed.

(*Part of definition of primitive poly*)
Lemma prim_fieldsize: 'qX ^+ 255 = 1 :> Q256.
Proof. by rewrite -qX_order_card // -(bijective_card byte_qpoly_bij) card_byte. Qed. 
  
(*Inverse calculation is correct (the key part of the inverse proof)*)
Lemma inv_calc b :
  b <> Byte.zero ->
  byte_pow_map (Byte.repr (fec_n - 1 - Byte.unsigned (byte_log_map b))) = 
  byte_inv b.
Proof.
  move => Hb. rewrite /byte_pow_map /byte_log_map /byte_inv /qpow_map_full /=.
  have /ltP Hbound: (nat_of_ord (dlog_alt 
             ((byte_to_qpoly b) : Q256)) < 256)%N. { 
    case : (dlog_alt (byte_to_qpoly b)) => [m Hm /=].
    move : Hm. by rewrite card_bool size_p256.
     } 
  rewrite Byte.unsigned_repr; last first.
  - remember (nat_of_ord (dlog_alt (byte_to_qpoly b))) as x.
    rewrite -Heqx. rep_lia.
  - f_equal. symmetry.
    apply: GRing.mulr1_eq. remember (byte_to_qpoly b) as q.
    have Hq0: q != 0. rewrite Heqq byte_zero_qpoly_iff.
    by apply /eqP. apply (dlog_alt_correct p256_geq_2) in Hq0.
    rewrite -{1}Hq0 -GRing.exprD.
    have->: 
      (dlog_alt (q : Q256) +
        Z.to_nat (fec_n - 1 - Z.of_nat (dlog_alt (q : Q256))))%N
      = 255%N.
    { rewrite Z2Nat.inj_sub; try rep_lia. rewrite Nat2Z.id /=.
      set x := (dlog_alt _ : nat) in Hbound *.
      rewrite fec_n_eq [Z.to_nat _]/= addnC subnK // -ltnS.
      by apply/ltP. }
    { apply prim_fieldsize. }
Qed.

Lemma inv_calc_zero : 
  byte_pow_map (Byte.repr (fec_n - 1 - Byte.unsigned (byte_log_map Byte.zero)))
  = Byte.one.
Proof.
  rewrite /byte_log_map /= byte_zero_qpoly /=. 
  have->:nat_of_ord (@dlog_alt _ _ p256_primitive 0) = 0%N.
  apply /eqP.
  ...
  rewrite Byte.unsigned_repr; try rep_lia.
  have->:Z.to_nat (fec_n - 1) = 255%N by rep_lia.
  apply prim_fieldsize.
Qed.

Lemma inv_calc_nonzero b :
  byte_pow_map (Byte.repr (fec_n - 1 - Byte.unsigned (byte_log_map b))) 
  <> Byte.zero.
Proof.
...
(*The proof of correctness for [populate_invs] is similar to the log part of  *)
(*the above, but a bit simpler                                                *)
Lemma populate_invs_aux_invar i base :
  0 <= i <= Byte.modulus ->
  Zlength base = Byte.modulus ->
  Znth 0 base = Byte.zero ->
  (Znth 0 (populate_invs_aux i base) = Byte.zero /\ 
  forall (b : byte), Byte.unsigned (byte_inv b) < i ->
...
Proof.
  ...
  - rewrite /populate_invs_aux /=. split =>[//|b].
    pose proof (@qpoly_to_Z_bound) ((byte_to_qpoly b) ^- 1). 
    set u := qpoly_to_Z _ in H *; lia.
  - have->: Z.of_nat n.+1 = (Z.of_nat n + 1) %Z by lia. move => Hn1 Hlen Hb0.
    ...
    have Hfst: Znth 0  (upd_Znth
     (Byte.unsigned
        (byte_pow_map (Byte.repr (fec_n - 1 - Byte.unsigned
          (byte_log_map (Byte.repr (Z.of_nat n)))))))
     (populate_invs_aux (Z.of_nat n) base) (Byte.repr (Z.of_nat n))) = 
     Byte.zero. {
      rewrite upd_Znth_diff //. apply IH => //. lia.
        rewrite populate_invs_aux_length. lia.
      rewrite populate_invs_aux_length; rep_lia.
      move => Hzero. apply esym in Hzero. 
    ...
    + rewrite {Hfst}. apply z_leq_n_1 in Hb. case : Hb => [Hbn | Hbn].
      * rewrite upd_Znth_diff. apply IH; try by []. lia. 
        ...
        move: Hbninv. rewrite inv_calc => Hinv.
        apply (congr1 GRing.inv) in Hinv.
        move: Hinv Hbn. rewrite GRing.invrK. have->: b^-1 = byte_inv b by [].
        move ->. rewrite Byte.unsigned_repr; try rep_lia.
        apply (congr1 Byte.unsigned) in Hinv.
        move: Hinv. rewrite Byte.unsigned_zero Byte.unsigned_repr; rep_lia.
      * rewrite inv_calc.
        { have->: Byte.unsigned b = 
                 (Byte.unsigned (byte_inv (Byte.repr (Z.of_nat n)))). {
           f_equal. rewrite -Hbn Byte.repr_unsigned.
           have->: byte_inv (byte_inv b) = b^-1^-1 by [].
           by rewrite GRing.invrK. }
          rewrite upd_Znth_same. by rewrite -Hbn Byte.repr_unsigned.
          rewrite populate_invs_aux_length; rep_lia.
        }
        { move => Hzero. apply (congr1 Byte.unsigned) in Hzero. move: Hzero.
          rewrite Byte.unsigned_repr; try rep_lia.
          rewrite Byte.unsigned_zero => Hn.
          rewrite Hn in Hbn. apply byte_unsigned_zero in Hbn. 
          have: byte_inv b <> Byte.zero. apply /eqP. apply GRing.invr_neq0.
          by apply /eqP. by [].
        }
Qed.

(*Full correctness*)
Lemma populate_invs_correct : populate_invs Byte.modulus = byte_invs.
Proof.
...
Lemma ord_byte_simpl (x : 'I_#|(byte: finType)|) :
  Byte.unsigned (ord_to_byte x) = Z.of_nat (nat_of_ord x).
Proof. by []. Qed.

Lemma nat_eq_ord n m (Hmn : m = n) (x : 'I_m) : (eq_ord Hmn x) = x :> nat.
Proof. by []. Qed.

(*A key result, needed in a few places:
  if a and b are equal mod 255, then x ^+ a = x^+ b*)
Lemma powX_eq_mod255 a b : 
  modn a 255%N = modn b 255%N -> ('qX ^+ a) = 'qX ^+ b :> Q256.
Proof.
have->:255%N=(#|(bool : finType)| ^ (size p256).-1).-1.
   by rewrite size_p256 card_bool.
move => Hab.
apply: powX_eq_mod =>//.
rewrite -(bijective_card byte_qpoly_bij) card_byte.
move: Hab.
by rewrite card_bool size_p256.
Qed.

(*Therefore, we can add the field size without changing anything*)
Lemma powX_add_255 a : 'qX ^+ a = 'qX ^+ (a + 255) :> Q256.
Proof. apply powX_eq_mod255. by rewrite addnC modnDl. Qed. 

(*The large case of multiplication*)
Lemma mult_large b1 b2 :
  fec_n - 1 < Byte.unsigned (byte_log_map b1) + Byte.unsigned (byte_log_map b2) 
  ->
  byte_pow_map (Byte.repr (Byte.unsigned (byte_log_map b1) + 
     Byte.unsigned (byte_log_map b2) - (fec_n - 1))) =
  byte_mul b1 b2.
Proof.
  rewrite /byte_log_map /byte_pow_map /byte_mul 
          /qpow_map_full !ord_byte_simpl !nat_eq_ord.
  move => Hbig. f_equal.
  (*so we don't have to do it twice*)
  have Hnz: forall (x y : byte), fec_n - 1 <
       Z.of_nat (dlog_alt ((byte_to_qpoly x) : Q256)) +
       Z.of_nat (dlog_alt ((byte_to_qpoly y : Q256))) ->
       x != 0. { move => x y Hbig'.
    case Hx: (x == 0) => [|//]. rewrite -byte_zero_qpoly_iff in Hx.
    apply (elimT eqP) in Hx. move: Hbig'.
    rewrite Hx. have->:nat_of_ord (dlog_alt (0 : Q256)) = 0%N.
      apply /eqP. by rewrite dlog_alt_zero. rewrite Nat2Z.inj_0 Z.add_0_l.
    case : (dlog_alt (byte_to_qpoly y)) => [m Hm].
    move: Hm. rewrite size_p256 card_bool /=.
    have->: (2^ 8)%N = 256%N by []. move => /ltP Hm. rep_lia. }
  have Hb1: b1 != 0. apply (Hnz b1 b2 Hbig).
  have Hb2: b2 != 0. rewrite Z.add_comm in Hbig. apply (Hnz b2 b1 Hbig).
   rewrite {Hnz}.
  rewrite -byte_zero_qpoly_iff in Hb1. rewrite -byte_zero_qpoly_iff in Hb2.
  apply (dlog_alt_correct p256_geq_2) in Hb1.
  apply (dlog_alt_correct p256_geq_2) in Hb2.
  rewrite powX_add_255 -{2}Hb1 -{2}Hb2 -GRing.exprD. f_equal. cbn.
  move: Hbig. set a := dlog_alt _ : nat; set b := dlog_alt _ : nat => Hbig.
  rewrite Byte.unsigned_repr.
  -have->:(Z.to_nat (Z.of_nat a + Z.of_nat b - (fec_n - 1)) + 255)%Nrec =
           (Z.to_nat (Z.of_nat a + Z.of_nat b - (fec_n - 1)) + 255)%coq_nat.
     by [].
    have->: (a + b)%Nrec = (a + b)%coq_nat by []. rep_lia.
  - move: Hbig. rewrite /a /b; case : dlog_alt => [a']. case : dlog_alt => [b'].
    rewrite size_p256 card_bool /=. have->:( 2 ^ 8)%N = 256%N by []. 
    move => /ltP Ha' /ltP Hb'. rep_lia.
Qed.

(*The small and easier case*)
Lemma mult_small b1 b2 :
  b1 <> Byte.zero ->
  b2 <> Byte.zero ->
  fec_n - 1 >= Byte.unsigned (byte_log_map b1) + Byte.unsigned (byte_log_map b2) 
  ->
  byte_pow_map (Byte.repr (Byte.unsigned (byte_log_map b1) + 
    Byte.unsigned (byte_log_map b2))) = byte_mul b1 b2.
Proof.
  move => /eqP Hb1 /eqP Hb2 Hsmall.
  rewrite -byte_zero_qpoly_iff in Hb1. rewrite -byte_zero_qpoly_iff in Hb2.
  apply (dlog_alt_correct p256_geq_2) in Hb1.
  apply (dlog_alt_correct p256_geq_2) in Hb2.
  move: Hsmall.
  rewrite /byte_log_map /byte_pow_map /byte_mul 
          /qpow_map_full !ord_byte_simpl !nat_eq_ord => Hsmall.
  rewrite -{2}Hb1 -{2}Hb2 -GRing.exprD. f_equal. f_equal. cbn. move: Hsmall.
  case : dlog_alt => [a]. case : dlog_alt => [b]. 
  rewrite  card_bool /=.
